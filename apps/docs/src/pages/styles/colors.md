...
Each palette color ships a full ramp — `50` through `900` — derived from the base
color and generated by `@rukkiecodes/tokens`, so a shade can never drift from the
color it belongs to. **Step `500` is the base color itself** (outlined below).

Reach for the ramp when a single hue is not enough: a `100` for a hover wash, a
`700` for text that has to stay legible on a pale fill.

<PaletteGrid />

They are emitted as CSS custom properties, as RGB triplets like every other color
token — so alpha works on them too:
...
/* The triplet is what makes this possible: */
.callout:hover {
  background: rgba(var(--fui-primary-100), 0.6);
}
```

They are also available at build time in SASS (`settings.$fui-shades`) and in
TypeScript:
...
The ramp is **not** theme-aware: unlike `--fui-theme-*`, these are fixed values
that do not change between light and dark. They are a palette to build with, not
a semantic role — reach for `--fui-theme-primary` when you mean "the brand color",
and for `--fui-primary-300` when you mean "that specific tint".
