/**
 * Text — FusionUI mobile typography primitive. size/level (H1–H6), weight, colour
...
const TextSkeleton: React.FC<{ width: number | string; height: number }> = ({ width, height }) => {
...
const TextComponent: React.FC<TextProps> & React.FunctionComponent<TextProps> = ({
  children,
  size,
  level,
  lineHeight,
  ...
  className,
}: TextProps): React.ReactNode & React.JSX.Element & React.ReactElement => {
  const theme = DEFAULT_THEME
  const fadeAnim = useRef<Animated.Value>(new Animated.Value(animated ? 0 : 1)).current
  ...
  const wrappedContent = onPress ? (
    <Pressable
      ...
      testID={testID}
      style={({ pressed }) => [containerStyles, pressed && { opacity: activeOpacity }]}
    >
      {content}
    </Pressable>
  ) : (
  ...
  if (animated) {
    return <Animated.View style={{ opacity: fadeAnim }}>{wrappedContent}</Animated.View>
  }

  return wrappedContent
}

const H1: React.FC<Omit<TextProps, 'level'>> & React.FunctionComponent<Omit<TextProps, 'level'>> = (
  props: Omit<TextProps, 'level'>
): (React.ReactNode & React.JSX.Element & React.ReactElement) | null => (
  <TextComponent {...props} level="h1" />
)
const H2: React.FC<Omit<TextProps, 'level'>> & React.FunctionComponent<Omit<TextProps, 'level'>> = (
  props: Omit<TextProps, 'level'>
): (React.ReactNode & React.JSX.Element & React.ReactElement) | null => (
  <TextComponent {...props} level="h2" />
)
const H3: React.FC<Omit<TextProps, 'level'>> & React.FunctionComponent<Omit<TextProps, 'level'>> = (
  props: Omit<TextProps, 'level'>
): (React.ReactNode & React.JSX.Element & React.ReactElement) | null => (
  <TextComponent {...props} level="h3" />
)
const H4: React.FC<Omit<TextProps, 'level'>> & React.FunctionComponent<Omit<TextProps, 'level'>> = (
  props: Omit<TextProps, 'level'>
): (React.ReactNode & React.JSX.Element & React.ReactElement) | null => (
  <TextComponent {...props} level="h4" />
)
const H5: React.FC<Omit<TextProps, 'level'>> & React.FunctionComponent<Omit<TextProps, 'level'>> = (
  props: Omit<TextProps, 'level'>
): (React.ReactNode & React.JSX.Element & React.ReactElement) | null => (
  <TextComponent {...props} level="h5" />
)
const H6: React.FC<Omit<TextProps, 'level'>> & React.FunctionComponent<Omit<TextProps, 'level'>> = (
  props: Omit<TextProps, 'level'>
): (React.ReactNode & React.JSX.Element & React.ReactElement) | null => (
  <TextComponent {...props} level="h6" />
)

export const Text = Object.assign(
  memo<React.FC<TextProps> & React.FunctionComponent<TextProps>>(TextComponent),
  {
    H1: memo<Omit<TextProps, 'level'>>(H1),
    H2: memo<Omit<TextProps, 'level'>>(H2),
    H3: memo<Omit<TextProps, 'level'>>(H3),
    H4: memo<Omit<TextProps, 'level'>>(H4),
    H5: memo<Omit<TextProps, 'level'>>(H5),
    H6: memo<Omit<TextProps, 'level'>>(H6),
  }
)
