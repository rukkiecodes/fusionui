    ...
    useRender(() => {
      ...
      const alertNode = h(
        'div',
        {
          ...
          class: [
            'vd-alert',
            `vd-alert--${props.variant}`,
            { 'vd-alert--collapsible': collapsible.value },
            { 'vd-alert--has-pages': totalPages.value > 0 },
            props.class,
          ],
          style: [
            { '--vd-alert-color': alertColor.value, '--vd-alert-on': alertOn.value },
            props.style,
          ],
          role: 'alert',
        },
        [
          icon.value || slots.icon
            ? h('div', { class: 'vd-alert__icon' }, [
                slots.icon ? slots.icon() : h(VdIcon, { icon: icon.value }),
              ])
            : null,
          ...
          hasTitle
            ? h(
                'div',
                {
                  class: ['vd-alert__title', { 'vd-alert__title--clickable': collapsible.value }],
                ...
                [
                  ...
                  collapsible.value
                    ? h(VdIcon, {
                        icon: '$dropdown',
                        class: ['vd-alert__chevron', { 'vd-alert__chevron--open': !hidden.value }],
                      })
                    : null,
                ]
              )
          ...
          hasContent
            ? h('div', { ref: contentRef, class: 'vd-alert__content' }, [
                h('div', { class: 'vd-alert__content__text' }, [
          ...
          props.closable
            ? h(
                'button',
                { class: 'vd-alert__close', type: 'button', 'aria-label': 'Close', onClick: close },
                [h(VdIcon, { icon: '$close' })]
              )
            : null,

          slots.footer ? h('div', { class: 'vd-alert__footer' }, [slots.footer()]) : null,

          Number(props.progress)
            ? h('div', { class: 'vd-alert__progress' }, [
                h('div', {
                  class: 'vd-alert__progress__bar',
          ...
          totalPages.value > 0
            ? h('div', { class: 'vd-alert__pagination' }, [
      ...
      return h(
        Transition,
        { name: 'vd-alert-collapse', onBeforeEnter, onEnter, onAfterEnter, onLeave },
