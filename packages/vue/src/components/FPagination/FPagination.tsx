  ...
  setup(props: any) {
    ...
    useRender(() =>
      h(
        'nav',
        {
          class: ['fui-pagination', { 'fui-pagination--disabled': props.disabled }, props.class],
          style: [{ '--fui-pagination-color': `var(--fui-theme-${props.color})` }, props.style],
          role: 'navigation',
        },
        [
          h(
            'button',
            {
              class: 'fui-pagination__btn fui-pagination__nav',
              disabled: page.value <= 1,
              'aria-label': 'Previous page',
              onClick: () => go(page.value - 1),
            },
            [h(FIcon, { icon: '$prev', size: 'small' })]
          ),
          ...items.value.map((item, i) =>
            item === 'ellipsis'
              ? h('span', { key: `e-${i}`, class: 'fui-pagination__ellipsis' }, '…')
              : h(
                  'button',
                  {
                    key: item,
                    class: [
                      'fui-pagination__btn',
                      { 'fui-pagination__btn--active': item === page.value },
                    ],
